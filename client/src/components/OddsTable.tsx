...
export function OddsTable({ events }: Props) {
  ...
  const style = {
    ...
    boxStyling2:
      'flex m-2 mt-2 text-odds xs:h-10 drop-shadow-xl justify-center items-center rounded-md bg-[#2e2e31] h-[28%] text-[#54D338] ;',
    date: 'flex text-smallest justify-center items-center h-1/4 border-b-[1px]',
    thead:
      'flex xs:text-xs text-smallest justify-center items-center h-1/4 xs:h-10 mb-5 border-b-[1px] border-b-[#343541] w-full',
  };
  ...
  const odds = events.map((event, index) => {
    ...
    return (
      <div
        key={index}
        className="flex justify-center mt-6 2xl: w-full h-56 xs:min-h-min">
        <div className="flex py-2 px-2  rounded-md bg-[#212123e3] w-[80%] xs:min-h-18 h-46 mt-2">
          <div className="flex-col  w-2/5 text-white text-xl xs:min-h-min">
            <span className={style.thead}>{formattedDateTime}</span>
            <span className="flex text-lg justify-center items-center h-1/3 xs:h-10 xs:text-sm">
              {nameOne}
            </span>
            <span className="flex text-lg justify-center items-center h-1/3 xs:text-sm">
              {nameTwo}
            </span>
          </div>
